const getAdminCheckoutAlertEmail = (adminName, employeeName, schoolName, category, scheduledEndTime) => {
    return `
        ...
        .badge { display: inline-block; padding: 6px 14px; border-radius: 6px; font-weight: 700; font-size: 13px; letter-spacing: 0.5px; margin-bottom: 24px; text-transform: uppercase; background: #fef9c3; color: #a16207; border: 1px solid #fef08a; }
        ...
        .card { background-color: #fafafa; padding: 24px; border-radius: 8px; border: 1px solid #e4e4e7; border-left: 4px solid #ca8a04; }
        ...
        @media (prefers-color-scheme: dark) {
            ...
            .card { background-color: #09090b !important; border-color: #27272a !important; border-left-color: #ca8a04 !important; }
            .badge { background: transparent !important; }
        }
        </style></head><body>
        <div class="container"><div class="badge">System Monitor</div>
        <h2>Overdue Check-Out Alert</h2>
        <p>Hello ${adminName},</p>
        <p><strong>${employeeName}</strong> has not checked out of their shift, which was scheduled to end 10 minutes ago. A reminder has been sent to the employee.</p>
        <div class="card">
        ...
        <p style="margin-top: 24px; font-size: 13px; color: #71717a;">No action is strictly required unless you need to verify approved overtime.</p>
        </div></body></html>
    `;
};
module.exports = getAdminCheckoutAlertEmail;