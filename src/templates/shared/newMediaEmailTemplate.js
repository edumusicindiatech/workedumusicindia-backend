const getNewMediaEmailTemplate = (adminName, employeeName, schoolName, band, fileCount) => {
    return `
    ...
    <h2>New Vault Media Upload</h2><p>Hello ${adminName},</p><p>New performance media has been successfully uploaded to the system and is ready for your review.</p>
    <div class="card">
        <div class="card-item"><span class="label">Uploaded By</span><div class="value">${employeeName}</div></div>
        
        <div class="card-item"><span class="label">School</span><div class="value">${schoolName}</div></div>
        
        <div class="card-item"><span class="label">Category</span><div class="value">${band}</div></div>
        
        <div class="card-item"><span class="label">Files Added</span><div class="value" style="color: #2563eb;">🎥 ${fileCount} Video(s)</div></div>
    </div>
    <p style="margin-top: 24px;">You can view and grade these videos by logging into the admin portal.</p>
    </div></body></html>
    `;
};

module.exports = getNewMediaEmailTemplate;