const getAdminStatusAlertEmailTemplate = (adminName, employeeName, schoolName, category, status, reason) => {
    return `
    ...
    .card { background-color: #fafafa; padding: 24px; border-radius: 8px; border: 1px solid #e4e4e7; border-left: 4px solid #ef4444; margin-bottom: 24px; }
    ...
    <div class="container"><div class="badge">Status Update: ${status}</div>
    <h2>Assignment Status Changed</h2><p>Hello ${adminName},</p><p><strong>${employeeName}</strong> has marked themselves as <strong>${status}</strong> for a specific assignment.</p>
    <div class="card">
...
module.exports = getAdminStatusAlertEmailTemplate;